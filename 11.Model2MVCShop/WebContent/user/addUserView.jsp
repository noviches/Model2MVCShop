		...
		<script type="text/javascript">
			...
			$( function(){
				...
				$("button.btn.btn-info").on("click", function(){
					popWin = window.open("/user/checkDuplication.jsp", "popWin", "left=300,top=200,width=780,height=130,marginwidth=0,marginheight=0,"+"scrollbars=no,scrolling=no,menubar=no,resizable=no");
				});
				*/
					...
					$.ajax(
							{
								...
								success : function(JSONData, status){
									...
									if(JSONData.userId == null) {
										$($("span[class='help-block'")[0]).children("strong").text("사용 가능한 아이디 입니다.").css("color", "");
									}else {
										$($("span[class='help-block'")[0]).children("strong").text("사용 불가능한 아이디 입니다.").css("color", "red");
									}
								}
							}
						);
			...
			//==> "비밀번호 일치 확인" Event 처리 및 연결
			function checkPwd(pwd1, pwd2) {
				if(pwd1 != pwd2) {
					$($("span[class='help-block'")[1]).children("strong").text("비밀번호가 일치하지 않습니다.").css("color", "red");
				}else {
					$($("span[class='help-block'")[1]).children("strong").text("").css("color", "");
				}
			}
			$( function(){
				$("#password2").on("change", function(){
					if($("#password").val() != null && $("#password").val() != '' && $("#password2").val() != null && $("#password2").val() != '') {
						checkPwd($("#password").val(), $("#password2").val());
					}else {
						$($("span[class='help-block'")[1]).children("strong").text("비밀번호를 입력하세요.").css("color", "red");
					}
				});
			});
		</script>
			...
			<form class="form-horizontal">
				<div class="form-group">
					...
					<div class="col-sm-4">
						...
						<span id="helpBlock" class="help-block">
							<!-- <strong class="text-danger">입력 전 중복확인부터..</strong> -->
				...
				<div class="form-group">
					...
					<div class="col-sm-4">
						...
						<span id="helpBlock" class="help-block">
							<strong class="text-danger">- 를 제외한 13자리를 입력하세요</strong>
						</span>
					</div>
				</div>
				<div class="form-group">
