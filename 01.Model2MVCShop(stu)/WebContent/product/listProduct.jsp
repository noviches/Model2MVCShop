					...
					<tr class="ct_list_pop">
						...
						<td align="left">
							...
							<% }else if(vo.getProTranCode().equals("1")) { %>
								구매완료
								<% if(request.getParameter("menu").equals("manage")) { %>
									<a href="updateTranCodeByProd.do?prodNo=<%= vo.getProdNo() %>&tranCode=2&menu=<%= request.getParameter("menu") %>">배송하기</a>
								<% } %>
				...
				<tr>
					<td align="center">
						<% for(int i=1; i<=totalPage; i++) { %>
							<a href="/listProduct.do?page=<%= i %>&menu=<%= request.getParameter("menu") %>
								<% if(request.getParameter("searchKeyword") != null) { %>
									&searchCondition=<%= request.getParameter("searchCondition") %>&searchKeyword=<%= request.getParameter("searchKeyword") %>
								<% } %>
							"><%= i %></a>
						<% } %>
			    	</td>
